...
/* NAVBAR SCROLL STATE */
  ...
  /* CARD 3D TILT */
  document.querySelectorAll('.team-card, .news-card, .bento-item, .card, .perk-card, .service-card').forEach(card => {
    card.addEventListener('mousemove', e => {
      const r = card.getBoundingClientRect();
      const x = (e.clientX - r.left) / r.width  - 0.5;
      const y = (e.clientY - r.top)  / r.height - 0.5;
      card.style.transform  = `perspective(800px) rotateX(${-y * 6}deg) rotateY(${x * 6}deg) translateY(-6px)`;
      card.style.boxShadow  = `${-x * 12}px ${-y * 12}px 32px rgba(0,64,204,0.18)`;
    });
  ...
  /* SECTION DOT NAV */
  const sections = document.querySelectorAll('section[id], section.hero, section.stats-bar, section.cinematic-section');
  ...
  lightbox.querySelector('.lightbox-prev').addEventListener('click', () => {
    ...
    lbImg.src = gallery[gIdx].src; lbCap.textContent = `${gIdx+1} / ${gallery.length}`;
  });
